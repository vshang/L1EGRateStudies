...
def drawPointsHists(saveName, h1, title1, xaxis, yaxis, new=False, plotDir='.', doFit=False) :
...
def make_jet_calibrations( c, fName, cut, plotBase ) :
    ...
    yBinning = array('f', [i*0.2 for i in range(101)])
    for eta in [['0.0', '0.3'], ['0.3', '0.6'], ['0.6', '1.0'], ['1.0', '1.5'], # Barrel
        ...
        for i in range(len(quantile_list)-1) :
            ...
            frac_cut = cut+"abs(jetEta)>=%s && abs(jetEta)<=%s" % (eta[0], eta[1])
            print(frac_cut)
            to_plot = 'genJet_pt/jetEt:jetEt'
            ...
            yaxis = "Gen Jet pT / Jet P_{T}"
            title1 = "L1GCTJet Calibration vs Reco Jet P_{T}"
            ...
            g = g_and_fit[0]
            f = g_and_fit[1]
            g.SetTitle('%i_absEta_%s_to_%s' % (i, eta[0].replace('.','p'), eta[1].replace('.','p') ) )
            g.SetName('%i_absEta_%s_to_%s' % (i, eta[0].replace('.','p'), eta[1].replace('.','p') ) )
            f.SetTitle('%i_absEta_%s_to_%s_fit' % (i, eta[0].replace('.','p'), eta[1].replace('.','p') ) )
            f.SetName('%i_absEta_%s_to_%s_fit' % (i, eta[0].replace('.','p'), eta[1].replace('.','p') ) )
            print(g)
            g.Write()
            f.Write()
            #x = ROOT.Double(0.)
